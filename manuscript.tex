...
Third, when inspecting the effect of systematic (under)sampling for a given time-aggregation scheme, it can be seen that the systematic sampling itself also introduces an upward bias for the estimates of $\beta_{0}$. That is, for a given aggregation scheme (row), the average estimate of $\beta_{0}$ shows a positive bias when sampling only every second, third, fifth, or tenth observation. This can be explained by the intermitted exposure variables $X$ that remain unobserved as a result of the systematic (under)sampling, and that act as confounders for the effect $\beta_{0}$. Combined, the time-aggregation and systematic sampling result in an average estimate for $\beta_{0}$ of $0.936$ (MC-SE $= 0.003$) with a coverage rate of the 95\% confidence intervals of 0.018 for the case of sampling every 10th occasion and time-aggregation over $m = 10$ periods, whereas the true population value is $\beta_{0} = 0.5$. 
 ...
 \begin{table}[]
    ...
    \begin{tabular}[t]{ccccccc}
        ...
        $m$ & Every & Bias (MC-SE) & Cover. (MC-SE) & & Bias (MC-SE) & Cover. (MC-SE) \\
        \midrule
...
Finally, for Scenario 3, the numerical results can be found in Table~\ref{tab:results-hats-beta0}. These results mirror that of the condition with systematically sampling every third observation in the $m = 3$ aggregated process of Scenario 2. For monthly measurement of variables that have been aggregated over weeks, the average estimate is $-0.299$, implying a bias of $-0.799$ (MC-SE $= 0.002$) with a coverage rate of zero. For the condition with yearly measurements of month-aggregated measurements, the bias is relatively similar. Note how the bias that is induced by the time-aggregation and systematic sampling appears to level off with increasing time-aggregation and time-lags between measurement occasions. That is, while time-aggregation and time-lags do lead to bias, the bias of estimates of $\beta_{0}$ for $m = 30$ (as in Scenario 3) is not much larger than the performance in a condition with $m = 3$ (as in Scenario 2), but is actually rather similar.  
 ...
 \begin{table}[]
    ...
    \begin{tabular}[t]{cccc}
        \toprule
        $m$ & Every & Bias (MC-SE) & Coverage\\
        \midrule
        1 & 1 & 0.498 (0.002) & 0.926\\
        7 & 4 & -0.301 (0.002) & 0.000\\
        7 & 102 & -0.299 (0.002) & 0.000\\
        30 & 12 & -0.208 (0.002) & 0.000\\
        365 & 1 & -0.177 (0.002) & 0.000\\
        \bottomrule
    \end{tabular}
...
Time-aggregation of measurements and low process coverage due to systematic sampling over time are two very common temporal features of panel data. A specific time-aggregation or systematic sampling scheme can be selected in the design of a longitudinal study for a diverse set of methodological and practical reasons \parencites{dollard_timing_2014}. For example, aggregation is sometimes used as a method for dealing with measurement error \parencite{rushton_behavioral_1983} and increasing the interval between repeated measures decreases participant burden while still enabling researchers to measure participants over extended periods of time. HIER OOK VERWIJZEN NAAR JANSSENS\_QUALITATIVE\_2018However, more generally, substantive literature and theories in the social sciences are notoriously uninformative about the time scale at which to measure a process of interest \parencite{collins_analysis_2006, dormann_optimal_2015}. For this reason, \textcite[p. 533]{mitchell_building_2001} have stated that decisions about the time scale of a study design are often ``(...) left to intuition, chance, convenience, or tradition''. Similarly, \textcite[p. S85]{collins_effect_2002} express concern about the lack of awareness about the impact of temporal design choices on the results of a study: \begin{quote} ``Scientific, theoretical, or conceptual, as opposed to logistical, reasons for the choice of temporal design are rarely mentioned, and equally rare is any discussion of how this choice might have affected the statistical and substantive conclusions drawn from the study.'' \end{quote}

The aim of this paper was therefore to explore and demonstrate the impact of time-aggregation and systematic sampling on a researchers ability to STUDY THE CAUSAL MECHANISM THAT UNDERLIES A PARTICULAR ERLATIONSHIP OF INTEREST. More psecifically,


identify and estimate the underlying effects of a time-varying exposure from panel data. While the topic of time-aggregation has received some attention in the econometric literature for dynamic models, it has been severely understudied in the social sciences. For relatively simple autoregressive processes such as the AR(1) and ARX(1) with one time-dependent covariate, there exists some analytical work on time-aggregation and systematic sampling. For example, it can be analytically derived how the autoregressive effect of a stationary disaggregated AR(1) process and the autoregressive effect and the effect of the time-dependent predictor of a disaggregated ARX(1) process are related to the parameters that can be obtained from aggregated data \parencite[cf.][]{amemiya_effect_1972, brewer_consequences_1973}. Furthermore, in the time-series literature it is well-described how parameters of dynamic models change when the time-interval between repeated measures changes \parencite[e.g., due to systematic sampling][]{}. However, deriving analytically their combined impact for slightly more complex models is challenging \parencite{amemiya_effect_1972, petkovic_aggregation_2009}. We therefore presented simulation results for three different scenarios, including a process with a time-varying exposure and outcome for $N = 1$, a process with time-varying exposure, outcome, and covariate for $N = 1$, and a process with a time-varying exposure, outcome, and covariate for multiple units but few repeated measures per unit. Our focus has been on the lag-0 causal effect of $X$ on $Y$, $\beta_{0_{(1)}}$. This effect is of interest for situations where the time interval is large but the causal effect under study is relatively rapid, or when there is an implicit time-lag in the measurements of variables \parencite[e.g., the exposure refers to something that happened \textit{the past week} and the outcome relates to something \textit{at the moment of measurement}/``right now'';][]{muthen_can_2024, mundlak_aggregation_1961}. 

THIS IS NOTHING NEW, VOELKLE\_ROLE\_2018 also discuss how reseracher ca narrive at different conclusoisn depending on the different dtaa collection designs. They state that discrete tiem analysis has its place in the case of euqally spaced measurements occasions iwth aahigh sampling frequency. In this paper, we demonstrate what can go wrong when this is mismatched. 

We have shown that a mismatch between the temporal characteristics of a causal process and the time-scale of the observed data due to time-aggregation or systematic sampling over time leads to a bias in estimates of $\beta_{0}$. Bias in estimates of $\beta_{0}$ occurs as a result of systematic sampling because the intermitted variables act as unobserved time-varying confounders. That is, under the systematic sampling schemes of Scenarios 1, 2, and 3 the observed data do not contain information about variables that confound the lag-0 effect, and hence this effect is unidentified. For time-aggregation, the bias occurs because time-aggregation changes the nature of the process itself \parencite{amemiya_effect_1972}. For example, aggregation of an AR(1) process over $m = 3$ time periods results in an ARMA process with moving average components from the three time points between measurement occasion $k$ and $k - m$. An AR(1) model would thus be misspecified for aggregated data leading to biased parameter estimates. It is also not possible to fit an appropriate ARMA model as no information is available in the aggregated data about the time points between the measurement occasion $k$ and $k - m$. Again, the effect $\beta_{0}$ is unidentified in the data. The simulation study showed that the direction and size of the bias, however, is very much dependent on the underlying causal process: We observed a moderately strong upwards bias for Scenario 1 (i.e., without time-varying covariate) but a strong negative bias for Scenarios 2 and 3. Scenario 3 represents a situation in which panel data is used to study a daily process. The mismatch of time scales was so sever that it resulted in an average estimated lag-0 effect of $X$ to $Y$ of -0.299 with a coverage rate of 0, while the true population effect was 0.5. 

\subsection{5.1 Implications}
In line with many previous methodological researchers on this topic, this paper demonstrates the fundamental importance of making time scales a central aspect of substantive theories \parencite[cf.][]{hamaker_within-between_2023, mitchell_building_2001, dormann_optimal_2015, collins_effect_2002}. While this point has been made predominantly when the interest was in identifying lag-1 cross-lagged effects in panel data or mediation analysis using SEM, this paper shows that it is at least equally important when the interest is in lag-0 causal effects. Without specificity of the temporal characteristics of a causal process, it is not possible to make an informed decision about how to optimally design an new panel study, nor is it possible to evaluate if an existing dataset contains the information needed to answer a research question. For this reason, \textcite{collins_analysis_2006} argues that peer-review journals should request substantive researchers to provide a justification grounded in theoretical arguments for the time scale that was used in a study, and suggests that this is a necessary step for gaining insight into why particular findings in the literature fail to replicate. We agree with this argument. THE EFFECT OF X ON Y DOES NOT EXIST. ONLY ONCE WE HAVE LINKED OUR QUESITON TO A TIME FRAME CAN WE GO ABOUT THINKIN HOW TO STUDY THIS EFFECT. OR, AS VOELKLE\_ROLE\_2018 STATES IT: "WITHOUT CONSIDERING TIEM, THE SUTDY OF PSCYHOLGOCIAL MEHCANISAM IS IMPOSSIBLE. 



A second implication is that for fast-changing time-varying exposures (i.e., fast in relation to the time interval between repeated measurements) it not recommended to ask participants to aggregate their values. The problem is that in the process of aggregation, information about the disaggregated process is lost, preventing researchers from identifying and estimating effects about the process at the disaggregated level.  NEED TO THINK ABOUT THIS MORE. 

\subsection{5.2 Limitations}
Our paper is based on the assumption that there exists a ``true'' underlying discrete-time process. For our motivating example, we assumed the social media-anxiety process manifests itself from day-to-day. This decision is of course arbitrary and the exact intervals at which the process operates is unknown. In fact, many processes are assumed to be continuous-time process, for which the concept of a ``true'' or ``natural'' time-interval is not applicable \parencite{dormann_optimal_2015}. 

Another limitation is that this papers the results of this simulation study are confined to the case of lag-0 effects. For lag-1 effects, the impact of time-aggregation and systamatic sampling might be different. For ... 



\subsection{5.3 Future research}
The collection or use of panel data without a theoretical motivation about the time scale at which the causal process of interest plays out has a high risk of biased results following analyses. A future research question is if such bias can be (partly) remedied with smarter research design or analyses. One area of interest in this regards is continuous-time modeling in combination with panel data that have been collected at varying time-intervals. \textcite{dormann_optimal_2015} already recommend to use varying time lags between repeated measures in the design of panel studies. They do so for a different reason, that is, for computing the time lag at which the cross-lagged effect of $X$ to subsequent $Y$ is the biggest. However, this might also be a recommendable step considering the issue of unmeasured intermitted variables as a result of systematic sampling and which confound the lag-0 effect $\beta_{0}$. In a research design with varying time lags, researchers can attempt to capture such intermitted variables in their sample. The continues-time framework might then allow researchers to take the varying time lags into account and adjust for the confounding effect of intermitted time-varying confounders. However, the continuous-time framework is not commonly used in the context of confounder adjustment, and future research is needed to evaluate how well such an approach might work. 

ALSO, WHILE AGGREGATED DATA MIGHT NOT BE INFORMATIVE FOR INVESTIGATING AN UNDERLYING CUASAL MECHANISM, IT CAN BE USEFUL FOR OTHER TYPES OF CUASAL QUESTIONS (I.E., FOR OTHER ESTIMANDS). FOR EXAMPLE, QUESTIONS AOBUT TREATMENT REGIMES. CAUSAL QUESOITN ARE NOT ASKED IN TERMS OF PARAMETERS, BUT RATHER AS CONTRASTS OF POTENTIAL OUTCOMES. 
